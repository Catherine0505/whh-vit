...
def train_collective_nofusion(dataset_train, dataset_val, train_bs, val_bs, save_weight_folder):
    ...
    if not os.path.exists(save_weight_path): 
        os.makedirs(save_weight_path, exist_ok=False)
    ...
    trainedlinearhead_save_path = \
        pathlib.Path(save_weight_path / "trainedlinearhead_weights" / path_identifier / "collective_nofusion_test" / f"lr{lr}")
    trainingcurve_save_path = \
        pathlib.Path(save_weight_path / "training_curve" / path_identifier / "collective_nofusion_test" / f"lr{lr}")
    ...
    for iter in range(num_iters):
        ...
        print(f"Training time for iter {iter}: {train_endtime - train_starttime} s")
        print(f"Validation time for iter {iter}: {val_endtime - val_starttime} s")
        ...
        if nondecreasing_count >= nondecreasing_threshold: 
            plt.plot(np.arange(iter + 1), training_losses, label = "Training Loss")
            plt.plot(np.arange(iter + 1), validation_losses, label = "Validation Loss")
            plt.legend()
            plt.xlabel("Iterations")
...
dataset_train = dataset.ChildSubjectGroupDatasetPreload(root_folder="/home/ml_group/misc/data", type="train")
dataset_val = dataset.ChildSubjectGroupDatasetPreload(root_folder="/home/ml_group/misc/data", type="val")
...
for train_batch in dataloader_train: 
    ...
    break 
train_collective_nofusion(dataset_train=dataset_train, dataset_val=dataset_val, train_bs=4, val_bs=4, 
