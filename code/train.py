...
############################################################################
#   Collective model with multi-view. Train by subject.
############################################################################

def train_collective(dataset_train, dataset_val, train_bs, val_bs, save_weight_folder):
    # print(dataset_train[0][0].shape)
    ...
    trainedlinearhead_save_path = \
        pathlib.Path(save_weight_path / "trainedlinearhead_weights" / path_identifier / f"lr{lr}")
    trainingcurve_save_path = \
        pathlib.Path(save_weight_path / "training_curve" / path_identifier / f"lr{lr}")
...
train_collective(dataset_train=dataset_train, dataset_val=dataset_val, train_bs=4, val_bs=4, 
                 save_weight_folder="/autofs/space/celer_001/users/catherine_gai/misc/model_weights_test")